...
class SubUnderBagging:
	...
	def train(self, data, label):
		...
		for j in range(self.Q):
			...
			all_idx = np.array(range(data_num))
			np.random.shuffle(all_idx)

			# compare k and class size
			if self.k_mode == 1:
				if k / 2 < min(pos_num, neg_num):
					sampling_idx = np.r_[all_neg_idx[:int(k / 2)], all_pos_idx[:int(k / 2)]]
				else:
					if neg_num > pos_num:
						sampling_idx = np.r_[all_neg_idx[:k - pos_num], all_pos_idx]
					else:
						sampling_idx = np.r_[all_neg_idx, all_pos_idx[:k - neg_num]]
			elif self.k_mode == 2:
				if neg_num > pos_num:
					sampling_idx = np.r_[all_neg_idx[:k], all_pos_idx]
				else:
					sampling_idx = np.r_[all_neg_idx, all_pos_idx[:k]]
	...
	def predict(self, test_data, P):
		...
		temp_result = np.zeros([P, self.T, test_num])
		all_pred = np.zeros([self.Q, test_num])

		for i_Q in range(self.Q):
			all_pred[i_Q, :] = self.model[i_Q].predict_proba(test_data)[:, 1]

		for i_P in range(P):
			rand_idx = np.random.permutation(self.Q)
			temp_result[i_P, :, :] = all_pred[rand_idx[:self.T], :]

		pred_result = np.mean(temp_result, 1)
